...
class RiotService {
    ...
    async fetchRecentMatchIds(puuid, region, count = 20) {
        const regional = getRegionalRouting(region);
    ...
    async fetchMatchDetails(matchId, region) {
        ...
        return await this.makeRequest(url, 3600000); // Cache matches 1 hour
    }
            ...
            try {
                ...
                if (matchIds && matchIds.length > 0) {
                    ...
                    for (const mId of matchIds) {
                        if (fetchedMatches.length >= 10)
                            break; // Limit to 10 recent SoloQ games
                        try {
